...
class User{
    ...
    public function __construct(){
         ...
         * @var array $config
         */
        $this->database = $database = new Medoo([
    ...
    public function Register(string $first_name, string $last_name, string $user_email, string $user_password, string $user_password_repeat){
        ...
        if($user_password == $user_password_repeat){
            ...
            if(ctype_alpha($first_name) == true AND ctype_alpha($last_name) == true){
                //check length
                if(strlen($first_name) >= 2 AND strlen($last_name) >= 2){
                    //check if email
                    if(filter_var($user_email, FILTER_VALIDATE_EMAIL)){
                        ...
                        try {
                            ...
                            throw new UserException($this->error = $e);
                        }
                        ...
                        throw new UserException($this->error = "Enter a valid email!");
                    }
                }else{
                    throw new UserException($this->error = "First name and Last name should be at least 2 characters!");
                }
            }else{
...
class Task extends User{
    ...
    public function SaveFile(array $file_temp){
        if(isset($file_temp['size']) AND $file_temp['name'] AND $file_temp['tmp_name']){
            ...
            throw new UserException($this->error = "This is not an upload!");
        }
    }
